...
7. Set `FIREBASE_PROJECT_ID` in `wrangler.toml` so the Worker can verify Firebase ID tokens before upload.
...
- The R2 worker now requires a valid Firebase ID token for upload endpoints.
